...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $.ajax({
            url: "<?php echo get_uri('estimate/print_estimate/' . $estimate_info->id . '/' . $estimate_info->public_key . '/0') ?>",
            dataType: 'json',
            success: function (result) {
                if (result.success) {
                    ...
                    const options = {
                        ...
                        jsPDF: { unit: 'pt', format: 'a4', orientation: 'portrait', putOnlyUsedFonts: true, format: 'letter', compressPDF: true}
                    };
                    ...
                    html2pdf().set(options).from(div).toPdf().get('pdf').then((pdf) => {
                        ...
                        for (let i in e) {
                           // e[i].height <= 18 ? pdf.deletePage(+i + 1) : null;
                        }
                        ...
                        for (let i = 1; i <= totalPages; i++) {
                            ...
                            pdf.text(15, pdf.internal.pageSize.getHeight() - 15, 'Data de Emissão ' + '<?php echo $estimate_info->estimate_date ?>' , 'left');
                            pdf.text(pdf.internal.pageSize.getWidth() - 80, pdf.internal.pageSize.getHeight() - 15, 'Página ' + String(i) + ' de ' + String(totalPages) + '', 'left');
                        }
                    }).outputPdf('blob', {filename: 'Proposta_<?php echo $estimate_info->id ?>.pdf'}).then((result) => {
                       
                        var fd = new FormData();
                        fd.append("file", result, 'Proposta_<?php echo $estimate_info->id ?>.pdf');
                        ...
                        $.ajax({
                            ...
                            contentType: false,
                            async: false,
                            url: "<?php echo get_uri('webhooks_listener/upload_file') ?>",
                            dataType: "json",
                            data: fd,
                            success: function (result) {
                                console.log(result)
